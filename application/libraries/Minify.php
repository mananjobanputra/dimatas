...
class Minify
{
	...
	public $target_js_dir="assets/js";
	/**
	 * Output css file name.
	 *
	 * @var string
	 */
	public $target_css_dir="assets/css";
	...
	public function __construct($config = array())
	{
		...
		$this->target_css_dir            = $this->ci->config->item('target_css_dir', 'minify') ?: $this->target_css_dir;
		$this->js_dir             = $this->ci->config->item('js_dir', 'minify') ?: $this->js_dir;
		$this->target_js_dir             = $this->ci->config->item('target_js_dir', 'minify') ?: $this->target_js_dir;
		...
		$this->minify_output           = $this->ci->config->item('minify_output', 'minify') ?: $this->minify_output;
	...
	private function _deploy_css($force = TRUE, $file_name = NULL, $group = NULL)
	{
		...
		if($this->minify_output==true){
			$content=$this->output_css($this->_css_file);
		}
		else{
			$content=$this->output_css($this->css_array[$group]);
		}
		return $content;
	...
	private function _deploy_js($force = FALSE, $file_name = NULL, $group = NULL)
	{
		...
		$content='';
		if($this->minify_output==true){
			$content=$this->output_js($this->_js_file);
		}
		else{
			$content=$this->output_js($this->js_array[$group]);
		}
		return $content;
	}
	...
	private function _set($name, $value)
	{
		switch ($name)
		{
			...
			$this->_js_file = $this->target_js_dir . '/' . $value;
			...
			$this->_css_file = $this->target_css_dir . '/' . $value;
	...
	private function output_css($files){
		$content='';
		if(!is_array($files)){
			$content.='<link href="' . base_url($files) . '" rel="stylesheet" type="text/css" />';

		}
		else{
			foreach ($files as $key => $value) {
				$content.='<link href="' . base_url($this->css_dir."/".$value) . '" rel="stylesheet" type="text/css" />';
			}
		}
		return $content;
	}
	private function output_js($files){
		$content='';
		if(!is_array($files)){
			$content.='<script type="text/javascript" src="' . base_url($files) . '"></script>';

		}
		else{
			foreach ($files as $key => $value) {
				$content.='<script type="text/javascript" src="' . base_url($this->js_dir."/".$value) . '"></script>';
			}
		}
		return $content;
	}
