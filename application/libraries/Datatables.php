<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return string
    */
    public function generate($output = 'json', $charset = 'UTF-8',$produce_output=false)
    {
      ...
      $this->get_ordering();
      $this->get_filtering();
      return $this->produce_output(strtolower($output), strtolower($charset),$produce_output);
    }
    ...
    private function get_filtering()
    {
      ...
      for($i = 0; $i < count($mColArray); $i++){
        if($mColArray[$i]['searchable'] == 'true' && $mColArray[$i]['search']['value'] != '' ){
          $name= $this->check_cType()?$this->select[$mColArray[$i]['data']]:$this->columns[$i];
          //$this->filter($name,$mColArray[$i]['search']['value']);
        }
      }
          ...
          if($mColArray[$i]['searchable'] == 'true' ){
           ...
           // if($this->check_cType()){
             if($mColArray[$i]['search']['value'] != ''){
              $sWhere .= $name . " LIKE '%" . $mColArray[$i]['search']['value'] . "%'";
            }
           // }
           // else{
    ...
    * @return mixed
    */
    private function produce_output($output, $charset,$print_query=false)
    {
      $aaData = array();
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        if($output == 'json')
        {
          ...
          $add_c=array_keys($this->add_columns);
          $columns=array_merge($this->columns,$add_c);
          $columns = array_values(array_diff($columns, $this->unset_columns));
          return array('aaData' => $aaData,'columns' => $columns);
        }
      }
