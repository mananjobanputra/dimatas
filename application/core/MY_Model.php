...
class MY_Model extends CI_Model
{
    ...
    public function get_by()
    {
      ...
      if($this->get_fields!=''){
        $this->_database->select($this->get_fields);
      }
    ...
    {
      $this->trigger('before_get');
      /* dump($this->_extra_fields);*/
      if($this->get_fields!=''){
        $this->_database->select($this->get_fields);
      }
      if($this->_extra_fields!=''){       
        $this->_database->select("*,".$this->_extra_fields,false);
      }
     ...
     */
    public function insert($data, $skip_validation = FALSE,$duplicate = false)
    {
    ...
    if ($data !== FALSE)
    {
      ...
      $sql =  "INSERT INTO ".$this->_table." (".implode(', ', $keys).") VALUES (".implode(', ', $values).") ON DUPLICATE KEY UPDATE ".implode(', ', $update_fields);
  ...
  else
  {
     ...
     */
    public function insert_many($data, $skip_validation = FALSE,$duplicate = false)
    {
      $ids = array();
      foreach ($data as $key => $row)
      {
        /*$ids[] = $this->insert($row, $skip_validation, ($key == count($data) - 1));*/
        $ids[] = $this->insert($row, $skip_validation,$duplicate);
      }
      return $ids;
    }
    /**
    ...
    protected function _set_where($params)
    {
     ...
     if (count($params) == 1 && is_array($params[0]))
     {
      ...
      {
        if (is_int($field) && is_array($filter)){
          /*dump($filter);*/
